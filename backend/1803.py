...
print(f"   Beta publish dir: {cfg.beta_publish_dir}")
...
def publish_beta_artifacts(
    ...
    """Mirror beta-facing CSV artifacts into one stable folder for the beta API."""
    os.makedirs(beta_publish_dir, exist_ok=True)
    ...
    for name in selected:
        ...
        if os.path.exists(src) and name not in seen_names:
            dst = os.path.join(beta_publish_dir, name)
...
pd.DataFrame(catalog_rows).to_csv(
    dynamic_catalog_path, index=False
)
...
# %%
# =============================================================================
# Cell 9: AutoEncoder Subsystem Scoring (v3 – Sensor Rankings + Downtime Exclusion)
# =============================================================================
...
# NEW: how many top contributing sensors to report per timestamp
if not hasattr(cfg, 'ae_top_k_sensors'):
    cfg.ae_top_k_sensors = 5  # report up to top-5 contributors
...
print(f"   Config: ae_top_k_sensors      = {cfg.ae_top_k_sensors}")
...
# --- 9) NEW: Beta Alert Episodes + Sensor-Level Alerts ---
...
published_beta_paths = publish_beta_artifacts(
    ...
    extra_paths=sensor_value_paths,
)
print(f"   📦 Published {len(published_beta_paths)} beta CSV artifacts to {cfg.beta_publish_dir}")
...
print("   📊 Output files: 8 separate analytics files + 1 combined + 1 summary")
print("   📈 Beta chart inputs: df_chart_data.csv + sensor_values_{system}.csv")
print("   📦 Beta API folder now receives a single mirrored artifact set")
